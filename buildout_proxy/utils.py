...
def compose_url(request):
    ...
    allowed_hosts = [
        e for e in settings.get("buildout_proxy.allow.hosts").splitlines() if e
    ]
    if "*" not in allowed_hosts:
        match_hosts = [re.match(h.replace("*", ".{1,}"), domain) for h in allowed_hosts]
    ...
    if domain in passwords:
        domain = "{password}@{domain}".format(password=passwords[domain], domain=domain)
...
def get_cache_path(directory, url):
    ...
    if os.path.exists(directory) is False:
        os.mkdir(directory)
    return os.path.join(directory, hashlib.md5(url.encode("utf8")).hexdigest())


def get_cache_file(request, url):
    """ Return the cached file for a given request and url """
    settings = request.registry.settings
    f_path = get_cache_path(settings.get("buildout_proxy.directory") or "/tmp", url)
    if not os.path.exists(f_path):
        ...
        match_hosts = [
            h
            for h in settings["buildout_proxy.cache"].keys()
...
def cache_file(request, url, f_path):
    ...
    if extends:
        content = smart_section_replacer(
            content, "extends", extends.splitlines(), new_extends
        )
...
def smart_section_replacer(text, section, old_elements, new_elements):
    ...
    for old, new in zip(old_elements, new_elements):
        ...
        new_text = "{0}{1}".format(
            new_text[0:position], new_text[position:].replace(old, new, 1)
        )
